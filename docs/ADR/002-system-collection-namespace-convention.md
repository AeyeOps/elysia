...
- `ELYSIACTL_CONFIG_[hash]` - User configuration storage
- `ELYSIACTL_METADATA__` - Collection metadata for preprocessing
- `ELYSIACTL_FEEDBACK_[hash]` - User feedback storage
- `ELYSIACTL_CHUNKED_[collection]` - Preprocessed data chunks

These collections contain system-critical data and implementation details that should not be exposed to end users.

### Problem Statement
1. **Security Risk**: Internal collection names were leaking through error messages, exposing implementation details like "Collection 'ELYSIACTL_CONFIG_1169a452fe5da387b9c7ce2e6dc4cf8b' not found"
2. **Namespace Collision**: No mechanism prevents users from creating collections that start with `ELYSIACTL_`, which could:
   - Interfere with system operations
   - Create security vulnerabilities
   - Cause confusion about collection ownership
   - Break system assumptions about collection purposes
3. **Implicit Convention**: The `ELYSIACTL_` prefix convention exists but isn't formally documented or enforced
...
**All collections with names starting with `ELYSIACTL_` are reserved for system use.**

This establishes `ELYSIACTL_` as a protected namespace with special handling throughout the system:
- **Creation**: Only system components can create `ELYSIACTL_` prefixed collections
...
The `@recoverable_endpoint` decorator (from ADR-001) specifically filters `ELYSIACTL_` prefixed collection names:

```python
if name and name.startswith("ELYSIACTL_"):
...
def validate_collection_name(name: str) -> None:
    ...
    if name.upper().startswith("ELYSIACTL_"):
        raise ValueError(
            "Collection names starting with 'ELYSIACTL_' are reserved for system use. "
...
|---------------|---------|---------|
| `ELYSIACTL_CONFIG_` | User configuration storage | `ELYSIACTL_CONFIG_1169a452` |
| `ELYSIACTL_METADATA_` | Collection metadata | `ELYSIACTL_METADATA__` |
| `ELYSIACTL_FEEDBACK_` | User feedback storage | `ELYSIACTL_FEEDBACK_abc123` |
| `ELYSIACTL_CHUNKED_` | Preprocessed data | `ELYSIACTL_CHUNKED_Products` |
...
- **Maintainability**: System can safely assume `ELYSIACTL_` collections have specific schemas
...
### Negative
- **Breaking Change**: If any users have existing collections starting with `ELYSIACTL_`, they'll need to rename them
...
- ✅ Error messages filter `ELYSIACTL_` prefixed collections (Phase 5)
...
4. Consider case-insensitive matching (ELYSIACTL_, elysia_, Elysia_)

### Phase 3: Migration Support (FUTURE)
- Tool to detect existing user collections with `ELYSIACTL_` prefix
...
### 1. Different Prefix
**Option**: Use `__SYSTEM_` or `_INTERNAL_` instead of `ELYSIACTL_`
**Rejected**: `ELYSIACTL_` is already established throughout the codebase
...
### 4. No Restriction
**Option**: Allow users to create `ELYSIACTL_` collections but document the convention
...
RESERVED_PREFIXES = ["ELYSIACTL_", "SYSTEM_"]  # Future-proof for other prefixes
...
async def preprocess(collection_name: str, ...) -> None:
    ...
    if collection_name.upper().startswith("ELYSIACTL_"):
        raise ValueError(
            f"Cannot preprocess '{collection_name}': "
            "Collections starting with 'ELYSIACTL_' are reserved for system use."
        )
...
The `ELYSIACTL_` namespace is formally reserved for system use. This convention:
