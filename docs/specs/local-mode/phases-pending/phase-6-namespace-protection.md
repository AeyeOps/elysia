...
## Objective
Implement validation to prevent users from creating collections with the reserved `ELYSIA_` prefix, as defined in ADR-002.

## Problem Summary
Users can currently create collections starting with `ELYSIA_`, which could interfere with system collections, create security vulnerabilities, and cause confusion about collection ownership. ADR-002 establishes this as a reserved namespace that must be protected.
...
**Add Before:**
```python
# Note: ELYSIA_CHUNKED_ collections are system collections, so skip validation
# validate_collection_name would reject these
# This is intentional - only system code can create ELYSIA_ collections
```
...
- `/opt/elysia/elysia/api/routes/user_config.py:490` - Creates ELYSIA_CONFIG
- `/opt/elysia/elysia/api/utils/feedback.py:19` - Creates ELYSIA_FEEDBACK
- `/opt/elysia/elysia/tree/tree.py:1995` - Creates system collections
- `/opt/elysia/elysia/preprocessing/collection.py:640` - Creates ELYSIA_METADATA
...
2. Test validation logic:
   - Test rejection of ELYSIA_ prefix (case-insensitive)
...
- [ ] Try to preprocess a collection named "ELYSIA_test"
- [ ] Verify error message is user-friendly
- [ ] Try variations: "elysia_test", "Elysia_test"
- [ ] Verify system can still create ELYSIA_ collections
...
async def test_preprocess_rejects_reserved_namespace():
    """Test that preprocessing rejects ELYSIA_ collections."""
    from elysia.preprocessing.collection import preprocess_async
    
    with pytest.raises(ValueError) as exc:
        await preprocess_async(
            collection_name="ELYSIA_user_collection",
...
- [ ] System can still create ELYSIA_ collections
...
**Request:** `preprocess("ELYSIA_MyData")`
**Response:** 
```
400 Bad Request
{
    "detail": "Collection names starting with 'ELYSIA_' are reserved for system use. Please choose a different name for your collection."
}
```
...
2. **System Bypass**: System code that creates ELYSIA_ collections should NOT call validation
...
5. **Backward Compatibility**: Existing ELYSIA_ system collections continue to work
